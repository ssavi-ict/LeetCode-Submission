...
public:
    ...
    bool checkPathExist(TreeNode * root, int leafMid){
        ...
        while(root && startFrom > 0){
            ...
            if(root->left->val == trackPath[startFrom - 1]) root = root->left;
            else if(root->right->val == trackPath[startFrom - 1]) root = root->right;
            else return false;
            startFrom--;
        }
        // cout<<"R "<<leafMid<<endl;
    ...
    int countNodes(TreeNode* root) {
        ...
        // int count = height + 1;
        while(height--){
            int leafMid = (leafRight + leafLeft)/2;
            if(checkPathExist(root, leafMid)) leafLeft = leafMid;
            else leafRight = leafMid;
        }
        return leafLeft;
    }
};